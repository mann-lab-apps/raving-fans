...
## 현재 프로젝트 상태 (Current Project Status - 2026-05-06)

1. **아키텍처**: 
    - FastAPI 백엔드를 제거하고 **Next.js Server Actions** 기반의 단일 스택(Backend-less)으로 통합되었습니다.
    - 데이터는 `data/visions.json` 파일에 직접 영속화됩니다.
2. **주요 기능**:
    - **다중 고객(Persona) 관리**: 동적으로 고객군을 추가/삭제/이름 수정할 수 있는 CRUD 기능이 완비되었습니다.
    - **PWA 지원**: `next-pwa`를 연동하여 설치 가능한 웹 앱으로 전환되었습니다. 아이폰 홈 화면 설치를 위한 아이콘 및 메타 태그가 설정되었습니다.
3. **기술적 특이사항**:
    - `next-pwa`와 Turbopack의 호환성 문제로 인해, 빌드 및 개발 환경에서 **Turbopack을 비활성화**(`--webpack` 플래그 사용)하고 있습니다.
